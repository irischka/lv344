...
//            { Currencies.US_DOLLAR, "MacBook", 602.00 },
    ...
    @Test(dataProvider = "productNames")
    public void checkProductToCompare(String partialProductName) {
        //
        // Precondition
        HomePage homePage = loadApplication();
        delayExecution(1000);
        //
        // Steps
        HomeMessagePage homeMessagePage = homePage.putToCompareProductByPartialName(partialProductName);
        delayExecution(1000);
        //
        // Check
        Assert.assertEquals(homeMessagePage.getAlertMessageText(),
        		String.format(homeMessagePage.EXPECTED_MESSAGE_COMPARE, partialProductName));
        delayExecution(1000);
        //
        // Return to previous state
        homePage = homeMessagePage.closeAlertMessage();
